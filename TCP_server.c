...
int main()
{
	...
	// hier werden alle aktiven Socken gespeichert für die TCP/IP Funktionen
	...
	while (bSchleifeAktiv)
	{
		...
		// Prüfung, wer spricht zu uns. Anwort kommt aus dem Select
		...
		// Loop für alle eingehenden Verbindungen
		for (int i = 0; i < copieSockets.fd_count; i++)
		{
			...
			if (copieSockets.fd_array[i] == listenSocket)
			{
					...
					// Neue Verbindung zur aktiven Socket List hinzufügen
